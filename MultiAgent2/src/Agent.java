...
public class Agent implements Runnable {
	...
	private int id,assignment,lastAgent,domainSize,maxConstraint;
	...
	public Agent(int id, Mailer mailer, HashMap<Integer, ConsTable> constraints,int lastAgent,int domainSize) {
		this.id = id;
		this.mailer = mailer;
				...
				if(mail instanceof CPA && !((CPA) this.mail).containsMe(id)) {
					...
					loop2:for( VarTuple v : ((CPA)mail).getCPA()) {       // iterate through CPA
						...
						if(constraints.containsKey(v.getI())) {           // if we are constraint together
								if(this.constraints.get(v.getI()).getConsTable(v.getJ(),assignment)) {   // if constraint table is true
